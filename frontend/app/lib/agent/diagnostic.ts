...
export async function runDiagnosticTurn(
  ...
  messages: ConversationMessage[],
): Promise<DiagnosticTurn & { sourcesRetrieved: number; retrievalError?: string }> {
  const intakeResult = intake(messages);

  // Retrieve manual chunks; degrade gracefully if the Moss sidecar is down so
  // the chat turn still completes (the model answers from its own knowledge).
  let docs: MossDoc[] = [];
  let retrievalError: string | undefined;
  try {
    docs = await retrieve(product.id, intakeResult);
  } catch (err) {
    retrievalError = err instanceof Error ? err.message : String(err);
    console.error("[diagnostic] retrieval failed, continuing with empty docs:", retrievalError);
  }

  const decision = await decide(product, messages, docs, intakeResult);
  const turn = respond(decision, docs);
  return { ...turn, sourcesRetrieved: docs.length, retrievalError };
}
